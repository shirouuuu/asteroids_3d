...
FLOAT Vector<FLOAT, N>::at(size_t i) const {
  return vector[i];
}
...
/*template <class FLOAT_TYPE, size_t N>
Vector<FLOAT_TYPE, N> operator/(FLOAT_TYPE scalar, const Vector<FLOAT_TYPE, N> vector2)
{
  if(scalar == 0){
  ...
  Vector<FLOAT_TYPE, N> retVector = {};
  for (size_t i = 0;i < N; i++){
    retVector[i] = vector2[i] / scalar;
  }
  return retVector;
}
*/
