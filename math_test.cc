...
//Test für square_of_length
TEST(MYTEST, SquareOfLength) {
  Vector<float, 3> v = {{3.0f, 4.0f, 0.0f}};
  EXPECT_FLOAT_EQ(v.square_of_length(), 25.0f);  // 3**2 + 4**2 = 9 + 16 = 25
}

// Test für length
TEST(MYTEST, Length) {
  Vector<float, 3> v = {{3.0f, 4.0f, 0.0f}};
  EXPECT_FLOAT_EQ(v.length(), 5.0f);  // sqrt(25) = 5
}

// Test für Skalarprodukt operator
TEST(MYTEST, DotProduct) {
  Vector<float, 3> v1 = {{1.0f, 2.0f, 3.0f}};
  Vector<float, 3> v2 = {{4.0f, -5.0f, 6.0f}};
  EXPECT_FLOAT_EQ(v1 * v2, (1.0f * 4.0f) + (2.0f * -5.0f) + (3.0f * 6.0f)); // 4 - 10 + 18 = 12
}
